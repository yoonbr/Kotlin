...
// abstract는 객체를 만들 수 없음
abstract class ItemVO{
    abstract val type:Int

    // static을 만들기 위한 Kotlin 문법
    companion object{
        val TYPE_HEADER = 0
        val TYPE_DATA = 1
    }
...
class DataViewHolder(view: View):RecyclerView.ViewHolder(view){
    val completeIconView = view?.findViewById<ImageView>(R.id.completedIconView)
...
class ToDoListActivity : AppCompatActivity() {
        ...
        override fun onBindViewHolder(
            ...
            if (itemVO.type == ItemVO.TYPE_HEADER) {
                ...
                if (dateItem.completed) {
                    viewHolder.completeIconView.setImageResource(R.drawable.icon_completed)
                } else {
                    viewHolder.completeIconView.setImageResource(R.drawable.icon)
                }
            }
        }
    }

    // RecyclerView의 항목을 장식할 클래스
    ...
    // 데이터를 읽어서 RecyclerView에 출력하는 함수
    fun selectDB(){
        list = mutableListOf()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        fab.setOnClickListener{
            ...
            // 하위 액티비티를 출력하고 하위 액티비티가 사라지면 onActivityResult를 호출하도록
            // 액티비티를 출력
